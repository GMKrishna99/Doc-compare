...
const applyStructuralHighlighting = (
  ...
  if (side === "left") {
    ...
    ownStructure.images.forEach((img: any) => {
      const existsInRight = otherStructure.images.some(
        (rightImg: any) => rightImg.src === img.src || rightImg.alt === img.alt
      );
    ...
    ownStructure.images.forEach((img: any) => {
      const existsInLeft = otherStructure.images.some(
        (leftImg: any) => leftImg.src === img.src || leftImg.alt === img.alt
      );
  ...
  if (side === "left") {
    ...
    ownStructure.tables.forEach((table: any) => {
      const existsInRight = otherStructure.tables.some(
        (rightTable: any) => rightTable.element === table.element
      );
    ...
    ownStructure.tables.forEach((table: any) => {
      const existsInLeft = otherStructure.tables.some(
        (leftTable: any) => leftTable.element === table.element
      );
...
const countStructuralChanges = (leftStructure: any, rightStructure: any) => {
  ...
  leftStructure.tables.forEach((table: any) => {
    const existsInRight = rightStructure.tables.some(
      (rightTable: any) => rightTable.element === table.element
    );
    if (!existsInRight) deletions++;
  });
  ...
  rightStructure.tables.forEach((table: any) => {
    const existsInLeft = leftStructure.tables.some(
      (leftTable: any) => leftTable.element === table.element
    );
    if (!existsInLeft) additions++;
  });
  ...
  const walker = document.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode: (node) => {
        ...
        if (
          parent?.closest("img, table, .diff-insert-block, .diff-delete-block")
        ) {
